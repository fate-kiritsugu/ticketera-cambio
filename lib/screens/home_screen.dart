  ...
  Future<void> _onPrintTap() async {
    if (_expression.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Realiza un cálculo antes de imprimir')),
      );
      return;
    }
    ...
    try {
      await PrinterService.instance.printTicket(
        ...
        resultText: _formatNum(resultado),
        fechaHora: fechaHora,
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Ticket enviado a imprimir')),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Guardado. Impresora no conectada: revisa Ajustes')),
        );
      }
    }
  }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Text('Imprimir', style: TextStyle(fontSize: 16)),
            ),
          ),
